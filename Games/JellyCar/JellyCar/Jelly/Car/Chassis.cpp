...
Chassis::Chassis(World *w, const ClosedShape& sA, const ClosedShape& sB,
	...
	SpringBody(w, sA, mpp, shapeK, shapeDamp, edgeK, edgeDamp, pos, 0.0f, scale, false)
{
	mShapeA = sA;
	mShapeB = sB;
...
void Chassis::accumulateExternalForces()
{
	...
	for (int i = 0; i < mShocks.size(); i++)
	{
		...
		Vector2 tp = mShocks[i].mTire->getDerivedPosition();
		Vector2 tv = mShocks[i].mTire->getDerivedVelocity();

		Vector2 force = VectorTools::calculateSpringForce(pmp, pmv, tp, tv, mShocks[i].mD, mShocks[i].mK, mShocks[i].mDamp);
		...
		mShocks[i].mTire->AddShockForce(force);
	}
