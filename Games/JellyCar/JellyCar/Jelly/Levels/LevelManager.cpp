...
bool LevelManager::LoadCompiledLevel(World *world, std::string levelName, std::string carFileName)
{
	...
	for (int i = 0; i < objectsCount; i++)
	{
		...
		if (objectsArray[i].isPlatform)
		{
			...
			gameBody->AddKinematicControl(new KinematicPlatform(gameBody->GetBody(), Vector2(bodyInfo.posX, bodyInfo.posY), end, seconds, offset));

			if (offset != 0.0f)
			{
				Vector2 newPos = Vector2(bodyInfo.posX, bodyInfo.posY).lerp(end, 0.5f + (sinf((PI / 2.0f) + (PI*2.0*offset))*0.5f));
				gameBody->GetBody()->setPositionAngle(newPos, VectorTools::degToRad(bodyInfo.angle), Vector2(bodyInfo.scaleX, bodyInfo.scaleY));
			}
		}
		...
		if (objectsArray[i].isMotor)
		{
			...
			gameBody->AddKinematicControl(new KinematicMotor(gameBody->GetBody(), rps));
		}

		gameBody->GetBody()->updateAABB(0.0f, true);

		worldLimits.expandToInclude(gameBody->GetBody()->getAABB().Min);
		worldLimits.expandToInclude(gameBody->GetBody()->getAABB().Max);
...
bool LevelManager::LoadLevel(World *world, std::string levelName, std::string carFileName)
{
	...
	for (ObjectNode; ObjectNode; ObjectNode = ObjectNode->NextSiblingElement())
	{
		...
		//position
		bodyInfo.posX = (float)atof(ObjectNode->Attribute("posX"));
		bodyInfo.posY = (float)atof(ObjectNode->Attribute("posY"));

		//angle
		bodyInfo.angle = (float)atof(ObjectNode->Attribute("angle"));

		//scale
		bodyInfo.scaleX = (float)atof(ObjectNode->Attribute("scaleX"));
		bodyInfo.scaleY = (float)atof(ObjectNode->Attribute("scaleY"));
			...
			while (Element != NULL)
			{
				...
				if (Element->Value() != NULL)
				{
					...
					if (strcmp(sKinematic, "PlatformMotion") == 0)
					{
						...
						end.X += atof(Element->Attribute("offsetX"));
						end.Y += atof(Element->Attribute("offsetY"));


						float seconds = atof(Element->Attribute("secondsPerLoop"));

						float offset = 0.0f;

						if (Element->Attribute("startOffset") != NULL)
						{
							offset = atof(Element->Attribute("startOffset"));
						}
					...
					{
						float rps = atof(Element->Attribute("radiansPerSecond"));
						gameBody->AddKinematicControl(new KinematicMotor(gameBody->GetBody(), rps));
					}
				}
	...
	_carPos.X = atof(CarNode->Attribute("posX"));
	_carPos.Y = atof(CarNode->Attribute("posY"));
	...
	finishX = atof(SettingsNode->Attribute("finishX"));
	finishY = atof(SettingsNode->Attribute("finishY"));
	fallLine = atof(SettingsNode->Attribute("fallLine"));
...
void LevelManager::LoadScores(std::string fileName)
{
	...
	for (ObjectNode; ObjectNode; ObjectNode = ObjectNode->NextSiblingElement())
	{
		...
		for (size_t i = 0; i < _levels.size(); i++)
		{
			if (_levels[i].name == levelName)
			{
				_levels[i].time = atof(ObjectNode->Attribute("score"));
				_levels[i].jump = atof(ObjectNode->Attribute("jump"));
			}
		}
	}
}
...
void LevelManager::ReadLevelData(std::string levelName)
{
	...
	for (ObjectNode; ObjectNode; ObjectNode = ObjectNode->NextSiblingElement())
	{
		...
		//position
		gameObject.posX = (float)atof(ObjectNode->Attribute("posX"));
		gameObject.posY = (float)atof(ObjectNode->Attribute("posY"));

		//angle
		gameObject.angle = (float)atof(ObjectNode->Attribute("angle"));

		//scale
		gameObject.scaleX = (float)atof(ObjectNode->Attribute("scaleX"));
		gameObject.scaleY = (float)atof(ObjectNode->Attribute("scaleY"));
			...
			while (Element != NULL)
			{
				...
				if (Element->Value() != NULL)
				{
					...
					if (strcmp(sKinematic, "PlatformMotion") == 0)
					{
						...
						gameObject.platformOffsetX = atof(Element->Attribute("offsetX"));
						gameObject.platformOffsetY = atof(Element->Attribute("offsetY"));


						gameObject.platformSecondsPerLoop = atof(Element->Attribute("secondsPerLoop"));

						gameObject.platformStartOffset = 0.0f;

						if (Element->Attribute("startOffset") != NULL)
						{
							gameObject.platformStartOffset = atof(Element->Attribute("startOffset"));
						}
					...
					if (strcmp(sKinematic, "Motor") == 0)
					{
						...
						gameObject.motorRadiansPerSecond = atof(Element->Attribute("radiansPerSecond"));
					}
				}
				Element = Element->NextSiblingElement();
			}
...
BodyObject LevelManager::ReadBodyData(std::string bodyName)
{
	...
	bodyObject.info.massPerPoint = atof(pElem->Attribute("massPerPoint"));
	bodyObject.info.edgeK = atof(pElem->Attribute("edgeK"));
	bodyObject.info.edgeDamping = atof(pElem->Attribute("edgeDamping"));

	//load color of the body
	if (pElem->Attribute("colorR") != NULL)
	{
		bodyObject.info.colorR = atof(pElem->Attribute("colorR"));
		bodyObject.info.colorG = atof(pElem->Attribute("colorG"));
		bodyObject.info.colorB = atof(pElem->Attribute("colorB"));
	}
	...
	if (pElem->Attribute("shapeMatching") != NULL)
	{
		...
		if (strcmp(sShapeMatching, "True") == 0)
		{
			...
			bodyObject.info.shapeK = atof(pElem->Attribute("shapeK"));
			bodyObject.info.shapeDamping = atof(pElem->Attribute("shapeDamping"));
		}
	}
	...
	if (pElem)
	{
		...
		bodyObject.info.pressure = atof(pElem->Attribute("amount"));
	}
	...
	{
		BodyPoint bodyPoint;
		bodyPoint.x = atof(PointNode->Attribute("x"));
		bodyPoint.y = atof(PointNode->Attribute("y"));

		if (PointNode->Attribute("mass") != NULL)
		{
			bodyPoint.mass = atof(PointNode->Attribute("mass"));
		}
		else
		{
			bodyPoint.mass = -1;
	...
	for (SpingNode; SpingNode; SpingNode = SpingNode->NextSiblingElement())
	{
		...
		bodySpring.k = atof(SpingNode->Attribute("k"));
		bodySpring.damp = atof(SpingNode->Attribute("damp"));
