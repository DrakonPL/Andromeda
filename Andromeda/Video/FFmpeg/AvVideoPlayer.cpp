...
public:
	...
	AvVideoData() : allocated(false), video_dst_bufsize(0),pts(-1),t(-1){}
};
...
public:
	...
	AvAudioData() : decoded_buffer_len(0){}
};
...
bool AvVideoPlayer::load(string fileName, bool stream){
	unload();
	...
	if (strstr(input_filename, "rtsp") != NULL)
	{
		AVDictionary *opts = 0;
		av_dict_set(&opts, "rtsp_transport", "tcp", 0);

		//open input file for context
		if (avformat_open_input(&fmt_ctx, input_filename, NULL, &opts) < 0)
		{
	...
	if (open_codec_context(&video_stream_idx, fmt_ctx, AVMEDIA_TYPE_VIDEO) >= 0) {
		...
		//create texture
		texture = TextureManager::Instance()->CreateEmpty("", width, height, TextureFilerType::NearestFilter,TextureColorType::Texture_RGB);
		
		//upload it to render manager system
		RenderManager::Instance()->CreateEmptyTexture(texture);
	}
	...
	next_seekTarget = 0;
	requestedSeek = true;
	decoderThread = thread(&AvVideoPlayer::run_decoder, this);
	
	return true;
}


bool AvVideoPlayer::setupAudioOut( int numChannels, int sampleRate ){
	output_setup_called = true;
...
void AvVideoPlayer::unload(){
	...
	while( audio_queue.size() > 0 ){
		AvAudioData * data = audio_queue.front();
		delete data;
		audio_queue.pop();
	}
	...
	for( AvVideoData * data : video_buffers ){
...
AvVideoPlayer::AudioResult AvVideoPlayer::audioOut(float *output, int bufferSize, int nChannels){
	AudioResult result{0,last_pts,last_t};
	if( !fileLoaded ){ return result; }
	if( !isPlaying ){ return result; }
	if( !output_setup_called ){
		//ofLogWarning() << "[AvVideoPlayer] audioOut() called, but audio was not set up. you must call either player.setupAudioOut() or, if you know what you are doing, set player.forceNativeAudioFormat=true" << endl;
		return result;
	}
...
bool AvVideoPlayer::decode_next_frame(){
	av_free_packet(&packet);
...
bool AvVideoPlayer::decode_video_frame( int & got_frame ){
	/* decode video frame */
...
bool AvVideoPlayer::decode_audio_frame( int & got_frame ){
...
bool AvVideoPlayer::queue_decoded_audio_frame_alocked(){
	...
	AvAudioData * data = new AvAudioData();
...
// this is _mostly_ only called from the special happy thread!
bool AvVideoPlayer::decode_until( double t, double & decoded_t ){
...
bool AvVideoPlayer::queue_decoded_video_frame_vlocked(){
	...
	AvVideoData * data = video_buffers[video_buffers_pos];
...
bool AvVideoPlayer::copy_to_pixels_vlocked( AvVideoData * data ){
	if( sws_context == NULL ){
		// Create context
	...
	if (pixelWidth != width || pixelHeight != height)
	{
		...
		pixels = (unsigned char *)av_malloc(av_image_get_buffer_size(AV_PIX_FMT_RGB24, width, height, 1));
	}
	...
	//marcin
	uint8_t * video_pixel_data = pixels;// video_pixels.getData();
...
long long AvVideoPlayer::av_time_to_millis( int64_t av_time ){
	if( audio_stream_idx >= 0 ){
...
int64_t AvVideoPlayer::millis_to_av_time( long long ms ){
	//TODO: fix conversion
...
void AvVideoPlayer::setPositionMS(long long ms){
...
int AvVideoPlayer::getPositionMS(){
	if( !fileLoaded ) return 0;
	int64_t ts = last_pts;
	return av_time_to_millis( ts );
}

float AvVideoPlayer::getPosition(){
	return duration == 0? 0 : (getPositionMS()/(float)duration);
}

void AvVideoPlayer::setPosition(float percent){
...
void AvVideoPlayer::setPaused(bool paused){
	isPlaying = fileLoaded?!paused:false;
}

void AvVideoPlayer::setLoop(bool loop){
	isLooping = loop;
}

void AvVideoPlayer::setVolume(float vol){
	this->volume = vol;
}

float AvVideoPlayer::getVolume(){
	return volume;
}

bool AvVideoPlayer::isLoaded(){
	return fileLoaded;
}

long long AvVideoPlayer::getDurationMs(){
	return duration;
}

bool AvVideoPlayer::getIsPlaying(){
	return isPlaying;
}

string AvVideoPlayer::getMetadata( string key ){
	if( fmt_ctx != NULL ){
...
map<string,string> AvVideoPlayer::getMetadata(){
...
Texture* AvVideoPlayer::getTexture() {
	return texture;
}

unsigned char* AvVideoPlayer::getPixels()
{
	return pixels;
}


int AvVideoPlayer::getFrameNumber(){
...
double AvVideoPlayer::getFps(){
	if( !isLoaded() ) return 0; 
	return av_q2d(video_stream->r_frame_rate); 
}

AvVideoData * AvVideoPlayer::video_data_for_time_vlocked( double t ){
	AvVideoData * data = video_buffers[0];
	double bestDistance = 10;
	
	bool needsMoreVideo = true;
	int j = 0, bestJ = 0;
	for( AvVideoData * buffer : video_buffers )
	{
...
void AvVideoPlayer::update(float dt)
{
	//marcin
...
//		lock_guard<mutex> lock(video_buffers_mutex);
		...
		AvVideoData * data = video_data_for_time_vlocked(request_t);
		...
		if( texturePts != data->pts )
		{
			...
			RenderManager::Instance()->UpdateTexture(texture, pixels);
			texturePts = data->pts;
		}
...
void AvVideoPlayer::run_decoder(){
...
string AvVideoPlayer::getFile(){
	return "";// fileNameAbs;
}

string AvVideoPlayer::getInfo(){
...
int AvVideoPlayer::getCurrentFrame() {
    return (int)(round(getPositionMS()/(1000.0/getFps())));
}

int AvVideoPlayer::getTotalNumFrames() {
    if( !isLoaded() ) return 0;
    return (int)(duration * 1000 * getFps());
}

void AvVideoPlayer::firstFrame(){
    if(duration>0) {
        setPositionMS(0.0);
    }
}

void AvVideoPlayer::nextFrame(){
    if(duration>0) {
        setPositionMS(getPositionMS() + 1000.0/getFps());
    }
}

void AvVideoPlayer::previousFrame(){
    if(duration>0) {
        setPositionMS(getPositionMS() - 1000.0/getFps());
    }}

float AvVideoPlayer::getHeight() const {
   return (float)height;
}

float AvVideoPlayer::getWidth() const {
    return (float)width;
}

void AvVideoPlayer::draw(float _x, float _y, float _w, float _h) {
    //getTexture().draw(_x,_y,_w,_h);
}

void AvVideoPlayer::draw(float _x, float _y) {
